...
export default function Watch() {
  ...
  let initialPostId = queryParams.get("postId");
  ...
  const {
    ...
    servers,
    serverLoading,
  } = useWatch(animeId, initialEpisodeId, initialPostId);
  const {
    autoPlay,
    setAutoPlay,
    autoSkipIntro,
  ...
  useEffect(() => {
    ...
    const isValidEpisode = episodes.some(ep => ep.slug === episodeId);
    
    // If missing or invalid episodeId, fallback to first
    if (!episodeId || !isValidEpisode) {
      const fallbackId = episodes[0].slug;
    ...
    const postIdParam = initialPostId ? `&postId=${initialPostId}` : "";
    const newUrl = `/watch/${animeId}?ep=${episodeId}${postIdParam}`;
                ...
                <div className="flex flex-col gap-y-2 bg-[#11101A] p-4">
                  ...
                  <div className="flex flex-wrap gap-4 max-[575px]:grid max-[575px]:grid-cols-3 max-[575px]:gap-3 max-[480px]:grid-cols-2">
                    {seasons.map((season, index) => (
                      <Link
                        to={`/watch/${animeId}?postId=${season.id}`}
                        key={index}
